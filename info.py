...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '5907466765').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1001692682311').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '1').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '5907466765').split()]
auth_channel = environ.get('AUTH_CHANNEL','-1002076796104')
...
STREAM_URL = environ.get("STREAM_URL", "https://papa-tera.onrender.com") # https://example.com/ (Your Stream Url)
...
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/baljeetsingh_11147')
CHNL_LNK = environ.get('CHNL_LNK', 'https://t.me/baljeetsingh_11147')
TUTORIAL = environ.get('TUTORIAL', 'https://t.me/baljeetsingh_11147')
...
FILE_STORE_CHANNEL = [int(ch) for ch in (environ.get('FILE_STORE_CHANNEL', '-1002119707654')).split()]
MELCOW_NEW_USERS = is_enabled((environ.get('MELCOW_NEW_USERS', "True")), True)
PROTECT_CONTENT = is_enabled((environ.get('PROTECT_CONTENT', "False")), False)
PUBLIC_FILE_STORE = is_enabled((environ.get('PUBLIC_FILE_STORE', "True")), True)

LANGUAGES = ["panjabi", "pan", "tamil", "tam" ,"english", "eng", "hindi", "hin", "telugu", "tel", "kannada", "kan"]
